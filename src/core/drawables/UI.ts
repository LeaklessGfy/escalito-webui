...
export default class UI {
  ...
  private constructor(
    private readonly time: GameObjects.Text,
    private readonly cash: GameObjects.Text
  ) {}
  ...
  static build(scene: Scene, settings: Settings): UI {
    ...
    const time = scene.add
      .text(settings.width - 5, 10, '00:00', style)
      .setOrigin(1, 0);
    const cash = scene.add
      .text(settings.width - 5, 30, '0$', style)
      .setOrigin(1, 0);
